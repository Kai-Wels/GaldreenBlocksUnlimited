...
public class CreateGaldreenBlockCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (player.getInventory().getItemInMainHand().getType() == Material.AIR) {
            player.sendMessage(GaldreenBlocksUnlimited.createChatMessage("Du musst ein Item in der Hand halten!"));
            return true;
        }
        if (args.length < 5) {
            player.sendMessage(GaldreenBlocksUnlimited.createChatMessage("Anzahl Argumente war falsch. Es müssten >5 sein. (x y z bool name)"));
            return true;
        }
        ...
        try {
            startLocation = new Location(player.getWorld(), Integer.parseInt(args[0]), Integer.parseInt(args[1]), Integer.parseInt(args[2]));
        } catch (NumberFormatException e) {
            player.sendMessage(GaldreenBlocksUnlimited.createChatMessage("Die Koordinate enthält etwas was keine Zahl ist."));
            return true;
        }
        ...
        if(args[3].equals("True")){
            ...
            player.sendMessage("Error in boolean!");
            return false;
        }
